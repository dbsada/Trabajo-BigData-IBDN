...
def compose_up(project_home, db):
    return f"cd {project_home} && docker compose --profile db_{db} up -d --build"
...
def wait_for_infra(cfg):
    ...
    mlflow_port = int(os.getenv('MLFLOW_PORT', '5002'))
    ...
    if not wait_for_http(f"http://{vm_ip}:{mlflow_port}/", timeout=60):
